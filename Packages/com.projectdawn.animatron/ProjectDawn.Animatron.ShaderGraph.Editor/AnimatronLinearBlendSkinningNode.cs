...
namespace ProjectDawn.ShaderGraph
{
    ...
    [Title("Input", "Mesh Deformation", "Animatron Linear Blend Skinning")]
    class AnimatronLinearBlendSkinningNode : AbstractMaterialNode, IGeneratesBodyCode, IGeneratesFunction, IMayRequireVertexSkinning, IMayRequirePosition, IMayRequireNormal, IMayRequireTangent
    {
        ...
        [SerializeField]
        private SkinningMode m_SkinningMode = SkinningMode.FourJoints;

        [EnumControl("SkinningMode")]
        public SkinningMode SkinningMode
        {
            get { return m_SkinningMode; }
            set
            {
                if (m_SkinningMode == value)
                    return;

                m_SkinningMode = value;
        ...
        public AnimatronLinearBlendSkinningNode()
        {
            name = "Animatron Linear Blend Skinning";
        ...
        public override void CollectShaderProperties(PropertyCollector properties, GenerationMode generationMode)
        {
            ...
            {
                case SkinningMode.Dynamic:
                    properties.AddShaderProperty(new Vector1ShaderProperty()
                    {
                        displayName = "Skin Blend Weight Count",
                        overrideReferenceName = "_SkinBlendWeightCount",
                        overrideHLSLDeclaration = true,
                        hlslDeclarationOverride = HLSLDeclaration.HybridPerInstance,
                        hidden = true,
                        value = 4
        ...
        public void GenerateNodeFunction(FunctionRegistry registry, GenerationMode generationMode)
        {
            ...
            {
                sb.AppendLine("uniform ByteAddressBuffer _AnimatronSkinMatrices;");
            });
            ...
            registry.ProvideFunction("LoadSkinMatrix", sb =>
            {
                ...
                using (sb.IndentScope())
                {
                    ...
                    sb.AppendLine("// _AnimatronSkinMatrices consists of float32");
                    sb.AppendLine("float4 p1 = asfloat(_AnimatronSkinMatrices.Load4(offset + 0 * 16));");
                    sb.AppendLine("float4 p2 = asfloat(_AnimatronSkinMatrices.Load4(offset + 1 * 16));");
                    sb.AppendLine("float4 p3 = asfloat(_AnimatronSkinMatrices.Load4(offset + 2 * 16));");
            ...
            registry.ProvideFunction(GetFunctionName(), sb =>
            {
                ...
                using (sb.IndentScope())
                {
                    ...
                    {
                        case SkinningMode.Dynamic:
                            sb.AppendLine("int skinBoneCount = (int)(UNITY_ACCESS_HYBRID_INSTANCED_PROP(_SkinBlendWeightCount,float));");
                            sb.AppendLine("for (int i = 0; i < skinBoneCount; ++i)");
                            break;
                        case SkinningMode.FourJoints:
        ...
        {
            return "Animatron_LinearBlendSkinning_$precision";
        }
    }
}
